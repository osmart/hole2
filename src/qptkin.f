...
C N.B. as file_type includes 'BINARY' then will open as binary
     ...
     &  'output kinemage', FOUT, LABORT, '.kin')
      IF (LABORT) GOTO 55555

C write header for plot file n.b. wordview needed to avoid clipping
      WRITE ( SOUT, '(A)') 
     &'@text ',
     &' Output of K option of program qpt_conv',
     &' Input file: '//FIN(1:INDEX(FIN,'     ')-1),
     &' To view in conjuction with molecule use editor ',
     &'   to merge output file with output of prekin.',
     &'@kinemage 1',
     &'@caption',
     &' qpt objects',
     &'@onewidth',
